...
use super::helpers::build_factory_name;
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) if e.name().as_ref() == b"component" => {
                ...
                if is_sector && code.as_ref() == Some(&sector_code.to_string()) {
                    ...
                    walk_component(&mut reader, &mut buf, (0.0, 0.0, 0.0), &mut stations, component_names, faction_names, None, None, None, None, false)?;
                    break;
                }
            }
            Ok(Event::Eof) => break,
...
/// Recursively walk components accumulating positions
fn walk_component(
                ...
                match e.name().as_ref() {
                    b"component" => {
                        ...
                        if in_connections && in_connection {
                            ...
                            for attr in e.attributes().flatten() {
                                match attr.key.as_ref() {
                                    b"class" => nested_class = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                    b"code" => nested_code = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                    b"macro" => nested_macro = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                    b"owner" => nested_owner = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                    b"name" => nested_name = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                    _ => {}
                                }
                            }
                            ...
                            if nested_class.as_deref() == Some("station") {
                                let acc2 = (acc.0 + current_offset.0, acc.1 + current_offset.1, acc.2 + current_offset.2);
                                walk_component(reader, buf, acc2, stations, component_names, faction_names, nested_code, nested_macro, nested_owner, nested_name, true)?;
                                component_depth -= 1;
                                continue;
                            }
                        }
                    ...
                    b"position" if in_offset => {
                        ...
                        for attr in e.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"x" => current_offset.0 = String::from_utf8_lossy(&attr.value).parse().unwrap_or(0.0),
                                b"y" => current_offset.1 = String::from_utf8_lossy(&attr.value).parse().unwrap_or(0.0),
                                b"z" => current_offset.2 = String::from_utf8_lossy(&attr.value).parse().unwrap_or(0.0),
                                _ => {}
                            }
                        }
                    }
                    b"source" => {
                        for attr in e.attributes().flatten() {
                            match attr.key.as_ref() {
                                b"entry" => source_entry = Some(String::from_utf8_lossy(&attr.value).to_string()),
                                b"nameindex" => source_nameindex = String::from_utf8_lossy(&attr.value).parse().ok(),
                                _ => {}
                            }
                        }
                    }
                    ...
                    b"component" => {
                        ...
                        if component_depth == 0 {
                            ...
                            if is_station {
                                ...
                                if let (Some(code), Some(macro_name)) = (station_code, station_macro) {
                                    let acc2 = (acc.0 + current_offset.0, acc.1 + current_offset.1, acc.2 + current_offset.2);

                                    let name = if let Some(custom) = station_custom_name {
                                        custom
                                    } else if let Some(entry) = source_entry {
                                        build_factory_name(&entry, source_nameindex.unwrap_or(1), component_names)
                                    } else {
                                        component_names.get(&macro_name.to_lowercase())
                                            .cloned()
