...
path.X4user <- "C:/Users/graid/Documents/Egosoft/X4"
...
path.CSVs <- "C:/Users/graid/Downloads/X4SaveGameAnalysisRelease/X4SaveGameAnalysis/data"

# The path where the output images and html files and visualization libraries will be saved
path.Output <- "C:/Users/graid/Downloads/X4SaveGameAnalysisRelease/X4SaveGameAnalysis/output"
...
# gather the log
if ("time"  %in% names(df.log))  df.log$time  <- suppressWarnings(as.numeric(df.log$time))
if ("money" %in% names(df.log))  df.log$money <- suppressWarnings(as.numeric(df.log$money)) else df.log$money <- NA_real_
if ("category" %in% names(df.log)) df.log$category[is.na(df.log$category)] <- ""
df.log <- unique(df.log[, intersect(c("time","category","title","text","money","component"), names(df.log))])
...
if (!"seller.proxy.id" %in% names(df.tradelog)) {
  df.tradelog$seller.proxy.id <- NA_character_
}

# extract follower IDs safely
followers <- if (exists("df.wings") && "follower" %in% names(df.wings)) {
  unique(na.omit(df.wings$follower))
} else {
  character(0)   # no followers → empty vector
}

# assign proxy sellers if followers exist
if (length(followers) > 0) {
  df.tradelog$seller.proxy.id[df.tradelog$seller %in% followers] <- 
    df.tradelog$seller[df.tradelog$seller %in% followers]
}
